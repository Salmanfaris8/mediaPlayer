...
const Category = ({setRemoveVideoResponseFromCategory,removeVideoResponseFromView}) => {
  ...
  const videoDropOverCategory= async (e,categoryId)=>{
    ...
    // pass a response to view componenet
    setRemoveVideoResponseFromCategory(response)
    // get all updated categories
    getAllCategory()
  }

  const categoryVideoDargStart = (e,categoryId,video)=>{
  ...
  return (
    <div>
      ...
      <h3 className='fw-bold fs-2'>All Categories</h3>
      <button onClick={handleShow} className='btn btn-danger rounded-circle ps-3 pe-3 fs-5 fw-bolder'>+</button>
      </div>
      ...
      <div className="container-fluid mt-3">
          {
            ...
            allCategories?.map((categoryDetails)=>(
              ...
              {/* Each catogory videos */}
              <div className="row mt-2">
                {
                  categoryDetails?.allVideos?.length>0 &&
                  categoryDetails?.allVideos?.map(video=>(
                    <div draggable={true} onDragStart={e=>categoryVideoDargStart(e,categoryDetails?.id,video)} key={video?.id} className="col-md-4">
      ...
      <Modal.Footer>
        ...
        <Button onClick={handleAddCategory} className='btn btn-dark'>Add</Button>
      </Modal.Footer>
      </Modal>
    </div>
  )
